...
// Darknet
//---------------------------------------------------------------------------
Darknet::Darknet(const char *cfg_file, int input_net_size, int class_num, float nms_thresh, float conf_thresh, torch::Device *device) {

	load_cfg(cfg_file);

	input_net_size_ = input_net_size;
...
torch::Tensor Darknet::transform(const cv::Mat& origin_image) {
    cv::Mat resized_image;
    cv::cvtColor(origin_image, resized_image, cv::COLOR_RGB2BGR);
    cv::resize(resized_image, resized_image, cv::Size(input_net_size_, input_net_size_));
    cv::Mat img_float;
    resized_image.convertTo(img_float, CV_32F, 1.0 / 255);
    auto img_tensor = torch::from_blob(img_float.data, {1, input_net_size_, input_net_size_, 3}).to(*_device);
...
void Darknet::TensorBoxToVector(torch::Tensor prediction, int width, int height,
                                std::vector<cv::Rect>& regions, std::vector<std::string>& labels, const std::vector<string>& labels_name) {


   torch::Tensor result =  write_results(prediction, class_num_, 0.1, nms_thresh_);
    ...
    else
    {
        ...
        float w_scale = float(width) / input_net_size_;
        float h_scale = float(height) / input_net_size_;
        ...
        {
            float object_confidence = result_data[i][5];
            float class_score  = result_data[i][6];
            float score = object_confidence * class_score;

            int class_id = int(result_data[i][7]);
            cv::Rect box;
            box.x = result_data[i][1];
            box.y = result_data[i][2];
            box.width = result_data[i][3] - result_data[i][1];
            box.height = result_data[i][4]  - result_data[i][2];

            regions.push_back(box);
            labels.push_back(labels_name[class_id]);
        }
